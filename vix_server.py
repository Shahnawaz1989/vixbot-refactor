...
def get_gann_row_from_json(cmp_price: float, midpoint: bool = False) -> dict:
    """
    midpoint=False  -> morning ORB Gann (normal)
    midpoint=True   -> MIDDAY ORB Gann table
    """
...
def read_gann_levels_from_excel(excel_path: Path):
    import pandas as pd

    print("[GANN-DEBUG] Excel path:", excel_path)
    print("[GANN-DEBUG] Sheets:", pd.ExcelFile(excel_path).sheet_names)
...
def calc_gann_levels_with_excel(
    cmp_price: float, side: str, excel_path: Optional[Path] = None
) -> Dict[str, float]:
    """
    NOW USING JSON LOOKUP INSTEAD OF EXCEL.

    side: "BUY" ya "SELL"
    JSON structure:
      - buy_entry, buy_t2, buy_t25, buy_t3, buy_t35, buy_t4
      - sell_entry, sell_t2, sell_t25, sell_t3, sell_t35, sell_t4
      - buy_sl, sell_sl
      - buy_entry_opp (J7), sell_entry_opp (M7)
      - buy_t15 (H8), sell_t15 (N8)
    """
    # Agar MIDDAY Excel path aaya hai to midpoint=True use karo
    midpoint = excel_path == GANNEXCELPATH_MIDDAY
    row = get_gann_row_from_json(cmp_price, midpoint=midpoint)

    if side == "BUY":  # BUY BO
        # BUYING SIDE (same as old Excel mapping)
        buy_entry = row["buy_entry"]        # H8
        buy_t2 = row["buy_t2"]              # J8
        buy_t3 = row["buy_t3"]              # J9
        buy_t4 = row["buy_t4"]              # J10
        buy_t25 = row["buy_t25"]            # H9
        buy_t35 = row["buy_t35"]            # H10

        # SELLING SIDE (opposite, high vol shift etc.)
        sell_entry = row["sell_entry_opp"]  # M7
        sell_t15 = row["sell_t15"]          # N8
        sell_t2 = row["sell_t2"]            # M8
        sell_t3 = row["sell_t3"]            # M9
        sell_t4 = row["sell_t4"]            # M10
        sell_t25 = row["sell_t25"]          # N9
        sell_t35 = row["sell_t35"]          # N10

    elif side == "SELL":  # SELL BO
        # SELLING SIDE normal
        sell_entry = row["sell_entry"]      # N8
        sell_t2 = row["sell_t2"]            # M8
        sell_t3 = row["sell_t3"]            # M9
        sell_t4 = row["sell_t4"]            # M10
        sell_t25 = row["sell_t25"]          # N9
        ...
        # BUYING SIDE opposite
        buy_entry = row["buy_entry_opp"]    # J7
        buy_t15 = row["buy_t15"]            # H8
        buy_t2 = row["buy_t2"]              # J8
        buy_t3 = row["buy_t3"]              # J9
        buy_t4 = row["buy_t4"]              # J10
        buy_t25 = row["buy_t25"]            # H9
        ...
        raise ValueError(
            f"Invalid side passed to calc_gann_levels_with_excel: {side}"
        )

    # SL logic: cross‑SL
    buy_sl = row["buy_sl"]     # = sell_entry (N8)
    sell_sl = row["sell_sl"]   # = buy_entry (H8)
    ...
    # Extra keys for HALF_GAP logic
    if side == "BUY":
        levels["sell_t15"] = float(sell_t15)  # N8
    else:  # SELL
        levels["buy_t15"] = float(buy_t15)    # H8

    return levels


def cut_dec(x: float) -> float:
    return float(int(x))   # decimal hatao, rounding nahi


# ========= GAP-DAY DETECTOR (FIRST 15-MIN vs PREV H/L) =========
def detect_gap_day(
    idxdf: pd.DataFrame,
    prev_high: float,
    prev_low: float,
    ...
    """
    GAP rule (tumhare logic ke hisaab se):

    - First 15-min candle: 09:15–09:30
    - GAP UP  : agar first_15_min ka LOW > previous day HIGH (1 tick bhi upar)
    - GAP DOWN: agar first_15_min ka HIGH < previous day LOW  (1 tick bhi neeche)
    - Warna    NO_GAP
    """
    if idxdf.empty:
...
def atr_tradingview_style(df: pd.DataFrame, length: int = 14) -> float:
    """
    df: columns ['h','l','c'] float, time-sorted
    TradingView-style ATR = RMA(TR, length)
    """
    df = df.copy()
    ...
    # kam se kam 'length' TR chahiye
    if df["tr"].count() < length:
        return 0.0

    atr = pd.Series(index=df.index, dtype=float)

    # pehle 'length' TR ka simple average = first ATR
    first_window = df["tr"].iloc[:length]
    atr.iloc[length - 1] = first_window.mean()

    # RMA / Wilder smoothing
    for i in range(length, len(df)):
        prev_atr = atr.iloc[i - 1]
        curr_tr = df["tr"].iloc[i]
        atr.iloc[i] = (prev_atr * (length - 1) + curr_tr) / length
...
def get_angel_atr_14(api, symbol_token: str, trade_date: str) -> float:
    try:
        ...
        # cutoff ko same timezone me lao
        cutoff_naive = pd.to_datetime(f"{trade_date} 09:30")
        cutoff = cutoff_naive.tz_localize(
            df["ts"].dt.tz) if df["ts"].dt.tz is not None else cutoff_naive

        df = df[df["ts"] <= cutoff]

        print("[ATR-15M-ROWS] trade_date=", trade_date,
              "rows_after_cutoff=", len(df),
              "from", df["ts"].min() if not df.empty else None,
              "to", df["ts"].max() if not df.empty else None)
...
def detect_half_gap(api, nifty_idxdf: pd.DataFrame, trade_date: str) -> Dict[str, Any]:
    """Half Gap Detection with Angel ATR 14 (15-min ATR)."""
    base = {
    ...
    first_candle = nifty_idxdf[(nifty_idxdf.index >= first_start) & (
        nifty_idxdf.index < first_end)]
    ...
    base.update(
        {
            ...
            "gap_pct": round(gap_pct, 2),      # multiple of ATR (absolute)
...
# ========= ORB RANGE / ATR (10:00–10:15 vs 15-min ATR) =========


def detect_orb_atr_ratio(nifty_idxdf: pd.DataFrame, atr_14: float) -> Dict[str, Any]:
    """
    10:00–10:15 ORB candle high-low / ATR14 ratio nikalta hai.
    atr_14 already detect_half_gap se aya hua 15-min ATR hai.
    """
    result = {
        "orb_high": 0.0,
        "orb_low": 0.0,
...
# ========= ORB BREAKOUT 15-MIN CANDLE / ATR (trigger 15m bar vs 15-min ATR) =========
...
def detectbreakout15matrratio(
    ...
    trigger_time = breakout 1-min time; isse uska 15m bucket nikalte hain.
    """
    result = {
        "bo15_high": 0.0,
        "bo15_low": 0.0,
    ...
    # trigger_time jis 15-min bar me aata hai uska start (e.g. 10:45 -> 10:30-10:45)
    ...
    # ===== DEBUG PRINT YAHAN CHIPKAO =====
    print("DEBUG-BO15-idx15-index:", idx15.index[:10])  # first 10 bars
    ...
    if bucket_start not in idx15.index:
        print("DEBUG-BO15-NEARBY-INDEX:", idx15.index[idx15.index.get_loc(
            bucket_start, method='nearest')-1: idx15.index.get_loc(bucket_start, method='nearest')+2])
        return result

    row = idx15.loc[bucket_start]
    print("DEBUG-BO15-ROW:", row)  # exact row values
    # ===== END DEBUG =====
    ...
    print(
        "[ORB-15M-BO-ATR-DEBUG]",
        "bucket_start=", bucket_start,
        "bo15_high=", bo_high,
        "bo15_low=", bo_low,
        "bo15_range=", bo_range,
        "atr_14=", atr_14,
        "ratio=", round(bo_ratio, 2),
        "high_vol=", is_high_vol,
    )
...
def get_midday_entry_start(trade_date):
    """
    MIDDAY ORB ke liye fixed entry start time.
    Example: 13:30 ke baad hi entries allow.
    """
    return datetime.combine(trade_date, datetime.strptime("13:30", "%H:%M").time())

# ========= ORB MARKING (10:00–10:15) + TRIGGER (10:15–12:30) =========


def get_marking_and_trigger(idxdf: pd.DataFrame) -> Dict[str, Any]:
    """
    10:00–10:15 candle ka high/low mark karta hai,
    phir 10:15–12:30 window me close-based breakout detect karta hai.
    ...
    # 10:00–10:15 marking (1-min data aggregate)
    ...
    # 10:15–12:30 → 15-min breakout window
    ...
    for ts, row in trigger_df15m.iterrows():
        ...
        print(
            f"ORB-15M-CHECK {ts} close {closeprice} high {high_price} low {low_price}")

        # BUY BO: close 15-min candle breakout, CMP = that candle HIGH
        if closeprice > marked_high:
            return {
                "status": "ok",
                "trigger_side": "BUY",
                "trigger_time": ts,
                "trigger_price": high_price,  # BUY BO = 15-min HIGH
                "marked_high": marked_high,
                "marked_low": marked_low,
            }

        # SELL BO: close 15-min candle breakout, CMP = that candle LOW
        if closeprice < marked_low:
            return {
                "status": "ok",
                "trigger_side": "SELL",
                "trigger_time": ts,
                "trigger_price": low_price,   # SELL BO = 15-min LOW
                "marked_high": marked_high,
                "marked_low": marked_low,
            }
